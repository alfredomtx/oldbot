...
class _SearchChatOffButton extends _AbstractImageSearchAction
{
    __Call(method, args*) {
    ...
    */
    resolveImageName()
    {
        ...
        if (OldBotSettings.settingsJsonObj.images.client.chat.chatOff) {
            return imageName := OldBotSettings.settingsJsonObj.images.client.chat.chatOff
        }

        if (WINDOWS_VERSION = "7") && (isTibia13()) {
            return imageName := "chat_off_windows7.png"
        }

        if (InStr(WINDOWS_VERSION, "Win_8")) && (isTibia13()) {
            return imageName := "chat_off_windows8.png"
        }
